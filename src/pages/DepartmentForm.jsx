...
function DepartmentForm(props) {
  ...
  const [stateDepartment, setStateDepartment] = useState(initialState);
  const [companies, setCompanies] = React.useState([]);

  const reset = () => {
    setStateDepartment(initialState);
  };


  const setKeyValue = (key, value) => {
    setStateDepartment((prevState) => ({
      ...prevState,
  ...
  function submitData() {
    ...
    if (
      !stateDepartment.display===null ||
      !stateDepartment.name ||
      !stateDepartment.company
    ) {
    ...
    axios({
      ...
      url: `${isEdit
        ? `${apis.server}/departments/${stateDepartment.id}`
        : apis.server + "/departments"
        }`,
      data: stateDepartment,
      headers: {
        Authorization: `Bearer ${localStorage.token ? localStorage.token : ""}`,
      },
    })
    ...
    <div>
      ...
      <div className="right-content">
        <div className="tittle-content">
          ...
          />{" "}
          <label>{isEdit ? "Edit" : "Add"} Department</label>
        </div>
                ...
                <div className="input-i">
                  ...
                  <div className="input-i-txt">
                    <Switch
                      value="start"
                      aria-placeholder=""
                      checked={stateDepartment.display}
                      onChange={(event) => {
                        setKeyValue('display', event.target.checked);
                      }}
